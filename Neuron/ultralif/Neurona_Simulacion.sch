v {xschem version=3.4.5 file_version=1.2
}
G {}
K {}
V {}
S {}
E {}
B 2 1270 -360 2070 40 {flags=graph
y1=-0.00011
y2=1.9
ypos1=-0.319016
ypos2=2.49867
divy=5
subdivy=1
unity=1
x1=1e-10
x2=5e-05
divx=5
subdivx=1
node="\\"Vout [V]; vout\\""
color=6
dataset=-1
unitx=1
logx=0
logy=0
digital=0
linewidth_mult=3.5}
B 2 1270 -1160 2070 -760 {flags=graph
y1=0.39986585
y2=0.8507535
ypos1=0
ypos2=2
divy=5
subdivy=1
unity=1
x1=1e-10
x2=5e-05
divx=5
subdivx=1


dataset=-1
unitx=1
logx=0
logy=0

color="6 4"
node="\\"Vm [V]; vm\\"
\\"Vg [V]; vg\\""
linewidth_mult=3.5}
B 2 1270 -760 2070 -360 {flags=graph


ypos1=0
ypos2=2
divy=5
subdivy=1
unity=1
x1=1e-10
x2=5e-05
divx=5
subdivx=1


dataset=-1
unitx=1
logx=0
logy=0
y1=-0.12
y2=1.1
color="6 6"
node="\\"Vg [V]; Vg\\";"
linewidth_mult=3.5}
B 2 470 -1160 1270 -760 {flags=graph
y1=1.5617195e-08
y2=3.6688814e-08
ypos1=0
ypos2=2
divy=5
subdivy=1
unity=1
x1=1e-10
x2=5e-05
divx=5
subdivx=1


dataset=-1
unitx=1
logx=0
logy=0
color=6
node="\\"Iext [A]; i(@iext[current])\\""
linewidth_mult=4}
N 780 -440 780 -430 {
lab=Vm}
N 600 -440 600 -430 {
lab=Vm}
N 460 -440 460 -420 {
lab=Vm}
N 690 -400 740 -400 {
lab=#net1}
N 780 -340 780 -310 {
lab=#net1}
N 690 -340 780 -340 {
lab=#net1}
N 690 -400 690 -340 {
lab=#net1}
N 700 -280 740 -280 {
lab=Vg}
N 600 -330 600 -280 {
lab=Vg}
N 460 -360 460 -330 {
lab=Vg}
N 460 -330 460 -280 {
lab=Vg}
N 780 -250 780 -220 {
lab=GND}
N 910 -250 910 -220 {
lab=GND}
N 1050 -260 1050 -220 {
lab=GND}
N 980 -350 980 -290 {
lab=V1}
N 980 -290 1010 -290 {
lab=V1}
N 980 -400 980 -350 {
lab=V1}
N 980 -400 1010 -400 {
lab=V1}
N 1050 -340 1050 -320 {
lab=Vout}
N 1050 -290 1060 -290 {
lab=GND}
N 1060 -290 1060 -220 {
lab=GND}
N 1050 -220 1060 -220 {
lab=GND}
N 410 -390 420 -390 {
lab=Vg}
N 1050 -470 1050 -430 {
lab=VDD}
N 460 -220 460 -210 {
lab=GND}
N 1050 -340 1120 -340 {
lab=Vout}
N 140 -360 140 -330 {
lab=VDD}
N 140 -270 140 -250 {
lab=GND}
N 780 -280 800 -280 {
lab=GND}
N 800 -280 800 -220 {
lab=GND}
N 910 -280 930 -280 {
lab=GND}
N 930 -280 930 -220 {
lab=GND}
N 1050 -400 1070 -400 {
lab=VDD}
N 1070 -470 1070 -400 {
lab=VDD}
N 1050 -470 1070 -470 {
lab=VDD}
N 340 -440 460 -440 {
lab=Vm}
N 340 -440 340 -370 {
lab=Vm}
N 340 -310 340 -240 {
lab=GND}
N 410 -330 460 -330 {
lab=Vg}
N 910 -350 910 -310 {
lab=V1}
N 910 -470 910 -430 {
lab=VDD}
N 800 -220 800 -200 {
lab=GND}
N 850 -280 870 -280 {
lab=Vg}
N 850 -320 850 -280 {
lab=Vg}
N 700 -320 850 -320 {
lab=Vg}
N 700 -320 700 -280 {
lab=Vg}
N 460 -330 600 -330 {
lab=Vg}
N 600 -440 780 -440 {
lab=Vm}
N 410 -390 410 -330 {
lab=Vg}
N 910 -470 1050 -470 {
lab=VDD}
N 910 -350 980 -350 {
lab=V1}
N 930 -220 1050 -220 {
lab=GND}
N 780 -370 780 -340 {
lab=#net1}
N 640 -400 690 -400 {
lab=#net1}
N 800 -220 910 -220 {
lab=GND}
N 1050 -370 1050 -340 {
lab=Vout}
N 780 -220 800 -220 {
lab=GND}
N 910 -220 930 -220 {
lab=GND}
N 460 -440 600 -440 {
lab=Vm}
N 600 -280 700 -280 {
lab=Vg}
N 600 -370 600 -330 {
lab=Vg}
N 910 -370 910 -350 {
lab=V1}
N 1120 -340 1170 -340 {
lab=Vout}
N 1120 -550 1120 -340 {
lab=Vout}
N 740 -590 800 -590 {
lab=#net2}
N 800 -590 800 -550 {
lab=#net2}
N 310 -440 340 -440 {
lab=Vm}
N 220 -440 250 -440 {
lab=#net3}
N 220 -550 710 -550 {
lab=#net3}
N 220 -550 220 -440 {
lab=#net3}
N 280 -480 870 -480 {
lab=Vg}
N 740 -550 740 -510 {
lab=GND}
N 280 -440 280 -380 {
lab=VDD}
N 460 -390 530 -390 {
lab=VDD}
N 530 -400 600 -400 {
lab=VDD}
N 530 -400 530 -390 {
lab=VDD}
N 850 -320 870 -320 {
lab=Vg}
N 870 -480 870 -320 {
lab=Vg}
N 780 -400 840 -400 {
lab=VDD}
N 740 -510 800 -510 {
lab=GND}
N 770 -550 800 -550 {
lab=#net2}
N 160 -440 220 -440 {
lab=#net3}
N 960 -550 1120 -550 {
lab=Vout}
N 800 -550 900 -550 {
lab=#net2}
N 800 -510 930 -510 {
lab=GND}
N 930 -550 930 -510 {
lab=GND}
N 870 -590 930 -590 {
lab=Vg}
N 870 -590 870 -480 {
lab=Vg}
C {devices/title.sym} 150 80 0 0 {name=l1 author="Stefan Schippers"}
C {sky130_fd_pr/pfet_01v8.sym} 440 -390 0 0 {name=M1
L=10
W=1
nf=1
mult=1
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} 620 -400 0 1 {name=M2
L=0.15
W=1.5
nf=1
mult=1
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} 760 -400 0 0 {name=M3
L=0.15
W=7.5
nf=1
mult=2
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/nfet_01v8.sym} 760 -280 0 0 {name=M4
L=4
W=1
nf=1 
mult=1
model=nfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/nfet_01v8.sym} 890 -280 0 0 {name=M5
L=0.15
W=1
nf=1 
mult=1
model=nfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} 1030 -400 0 0 {name=M6
L=0.15
W=2
nf=1
mult=1
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/nfet_01v8.sym} 1030 -290 0 0 {name=M7
L=0.15
W=1
nf=1 
mult=1
model=nfet_01v8
spiceprefix=X
}
C {devices/isource.sym} 460 -250 0 0 {name=I1 value=10nA}
C {devices/gnd.sym} 460 -210 0 0 {name=l2 lab=GND}
C {devices/gnd.sym} 800 -200 0 0 {name=l3 lab=GND}
C {devices/lab_pin.sym} 1160 -340 3 1 {name=p2 sig_type=std_logic lab=Vout
}
C {devices/vsource.sym} 140 -300 0 0 {name=Vspk value=1.8}
C {devices/gnd.sym} 140 -250 0 0 {name=l5 lab=GND}
C {devices/lab_pin.sym} 660 -440 1 0 {name=p4 sig_type=std_logic lab=Vm}
C {sky130_fd_pr/corner.sym} 210 -1070 0 0 {name=CORNER only_toplevel=true corner=tt}
C {devices/code_shown.sym} 120 -880 0 0 {name=STIMULI 
only_toplevel=true
place=end
value="
.options savecurrents
*.nodeset v(vm)=0.75
.tran 10n 50u uic
.control
	run
	write Neurona_Simulacion.raw
.endc
"}
C {devices/vdd.sym} 140 -360 0 0 {nname=l4 lab=VDD}
C {devices/vdd.sym} 960 -470 0 0 {name=l7 lab=VDD}
C {devices/capa.sym} 340 -340 0 0 {name=C1
m=1
value=0.1p
footprint=1206
device="ceramic capacitor"}
C {devices/gnd.sym} 340 -240 0 0 {name=l8 lab=GND}
C {devices/isource.sym} 160 -470 0 1 {name=Iext value=30n}
C {devices/vdd.sym} 160 -500 0 0 {name=l4 lab=VDD}
C {devices/lab_pin.sym} 700 -280 3 0 {name=p1 sig_type=std_logic lab=Vg}
C {devices/launcher.sym} 530 -660 0 0 {name=h5
descr="load waves" 
tclcommand="xschem raw_read $netlist_dir/Neurona_Simulacion.raw tran"
}
C {devices/lab_pin.sym} 940 -350 1 1 {name=p3 sig_type=std_logic lab=V1
}
C {devices/isource.sym} 910 -400 0 0 {name=I3 value=100nA

spice_ignore=false}
C {sky130_fd_pr/nfet_01v8.sym} 740 -570 1 0 {name=M8
L=0.15
W=1
nf=1 
mult=1
model=nfet_01v8
spiceprefix=X
}
C {devices/gnd.sym} 800 -510 0 0 {name=l6 lab=GND}
C {sky130_fd_pr/pfet_01v8.sym} 280 -460 3 1 {name=M9
L=0.15
W=2
nf=1
mult=1
model=pfet_01v8
spiceprefix=X
}
C {devices/vdd.sym} 280 -380 2 0 {name=l9 lab=VDD}
C {devices/vdd.sym} 530 -390 2 0 {name=l10 lab=VDD}
C {devices/vdd.sym} 840 -400 2 0 {name=l11 lab=VDD}
C {sky130_fd_pr/nfet_01v8.sym} 930 -570 1 0 {name=M10
L=0.15
W=1
nf=1 
mult=1
model=nfet_01v8
spiceprefix=X
}
